...
def next_message_id(current_msg_id: tuple[bytes] = (0, 0)) -> tuple[bytes]:
...
def _create_command_encoding(cmd_id: int, cmd_mode: int, msg_id: tuple[bytes], parameters: list[int]) -> bytearray:
    ...
    command = bytearray([cmd_id, 1, len(parameters) + 5,
                        msg_id[0], msg_id[1], cmd_mode] + sanitized_params)
...
def create_switch_to_auto_mode_command(msg_id: tuple[bytes]) -> bytearray:
